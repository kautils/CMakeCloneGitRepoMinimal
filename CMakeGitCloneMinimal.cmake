...
macro(CMakeGitCloneMinimal prfx)
    ...
    if(${${prfx}_VERBOSE_GIT})
        set(__verbose_option VERBOSE VERBOSE_ERROR)
    endif()
    ...
    if(${__repo_force_update} OR (${${__result_var}} STREQUAL "") OR (NOT EXISTS ${${__result_var}}/.git))
        ...
        CMakeExecuteGit(execgit COMMANDS git init DIR ${__dest_c} ${__verbose_option})
        CMakeExecuteGit(execgit COMMANDS git remote add origin ${__repo_uri} DIR ${__dest_c} ${__verbose_option})
        
        if(DEFINED __repo_tag)
            CMakeExecuteGit(execgit COMMANDS git fetch ${__repo_remote} --tags ${__repo_tag} --depth=1 DIR ${__dest_c} ${__verbose_option})
            CMakeExecuteGit(execgit COMMANDS git checkout tags/${__repo_tag} DIR ${__dest_c} ${__verbose_option})
            CMakeExecuteGit(execgit COMMANDS git rev-list -n 1 ${__repo_tag} DIR ${__dest_c} ${__verbose_option})
            set(__want_hash ${execgit_OUTPUT_VARIABLE})
        elseif(DEFINED __repo_branch)
            CMakeExecuteGit(execgit COMMANDS git fetch ${__repo_remote} ${__repo_branch} DIR ${__dest_c} ${__verbose_option})
            CMakeExecuteGit(execgit COMMANDS git checkout ${__repo_branch} DIR ${__dest_c} ${__verbose_option})
            CMakeExecuteGit(execgit COMMANDS git rev-list -n 1 ${__repo_branch} DIR ${__dest_c} ${__verbose_option})
            set(__want_hash ${execgit_OUTPUT_VARIABLE})
        elseif(DEFINED __repo_digest)
            CMakeExecuteGit(execgit COMMANDS git fetch ${__repo_remote} ${__repo_digest} DIR ${__dest_c} ${__verbose_option})
            CMakeExecuteGit(execgit COMMANDS git checkout ${__repo_digest} DIR ${__dest_c} ${__verbose_option})
